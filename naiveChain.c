...
for (block in sp->blockList; block != NULL; block = block -> next)
{
    blockSize = block->qEnd - block->qStart
    checkBlockRange("query", qSeq, block->qStart, block->qEnd);
    checkBlockRange("target", tSeq, block->tStart, block->tEnd);
    block->score = axtScoreUngapped(scoreScheme, qSeq->dna + block->qStart, tSeq->dna + block->tStart, size);
}
...
while(b!=NULL)
    {
        struct cBlock *temp;
        while (b->next != NULL) {
            b->qStart = b->next->qStart
            b->qEnd = b->next->qEnd
            b->tStart = b->next->tStart
            b->tEnd = b->next->tEnd
            temp = b
            b = b->next
        }
...
for (chain = chainLists; chain != NULL; chain = chain->next)
    {
    chainRemovePartialOverlaps(chain, qSeq, tSeq, scoreScheme->matrix);
    chainKeepAllBlocks(chain, qSeq, tSeq, scoreScheme->matrix)
...
/*
for (chainList = chainLists; chainList != NULL; chainList = chainList->next)
    {
        ...
        // Move chains scoring over threshold to master list and add 
        for (chain = chainList; chain != NULL; chain = next)
        {
            next = chain->next;
            if (chain->score >= minScore)
